...
void arm_exec_pixels(uint32_t target_cycles, arm_pixel_fn fn) {
    if (int_halt) {
        timers_clock(target_cycles);
    ...
    while (arm_cycles < target_cycles) {
        ...
        if (fn) {
            uint32_t prev_px = cycles / 4;
            uint32_t curr_px = arm_cycles / 4;
            if (curr_px > prev_px) {
                for (uint32_t p = prev_px; p < curr_px && p < 240; p++)
                    fn((int)p, v_count.w);
            }
        }
    }
