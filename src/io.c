...
uint8_t io_read(uint32_t address) {
    ...
    switch (address) {
        ...
        case 0x04000040: return win0_h.b.b0          & 0xff;
        case 0x04000041: return win0_h.b.b1          & 0xff;
        case 0x04000042: return win1_h.b.b0          & 0xff;
        case 0x04000043: return win1_h.b.b1          & 0xff;
        case 0x04000044: return win0_v.b.b0          & 0xff;
        case 0x04000045: return win0_v.b.b1          & 0xff;
        case 0x04000046: return win1_v.b.b0          & 0xff;
        case 0x04000047: return win1_v.b.b1          & 0xff;
...
static void snd_reset_state(uint8_t ch, bool enb) {
    if (enb) {
        ...
        if (ch < 2) {
            ...
            st->length_cnt    = (sqr_ch[ch].ctrl.w & CH_LEN) ? (64 - snd_len) * 128 : 0xFFFFFFFF;
            st->envelope      = (sqr_ch[ch].tone.w >> 12) & 0xf;
            st->sweep_cnt     = (ch == 0) ? (((sqr_ch[ch].sweep.w >> 4) & 7) + 1) * 256 : 0;
            st->env_cnt       = env_step ? env_step * 512 : 0;
        } else if (ch == 2) {
            uint8_t snd_len = (wave_ch.volume.w >> 0) & 0xff;
            st->length_cnt = (wave_ch.ctrl.w & CH_LEN) ? (256 - snd_len) * 128 : 0xFFFFFFFF;
            wave_reset();
        } else {
            uint8_t snd_len   = (noise_ch.env.w >> 0) & 0x3f;
            uint8_t env_step  = (noise_ch.env.w >> 8) & 0x7;
            st->length_cnt    = (noise_ch.ctrl.w & CH_LEN) ? (64 - snd_len) * 128 : 0xFFFFFFFF;
            st->envelope      = (noise_ch.env.w >> 12) & 0xf;
            st->env_cnt       = env_step ? env_step * 512 : 0;
            st->lfsr          = (noise_ch.ctrl.w & NOISE_7) ? 0x007f : 0x7fff;
        }
...
void io_write(uint32_t address, uint8_t value) {
    switch (address) {
        ...
        case 0x0400004c: mosaic.b.b0          =  value; break;
        case 0x0400004d: mosaic.b.b1          =  value; break;
